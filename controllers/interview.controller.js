...
import { createAndReturnNewInterviewDataDocument, updateInterviewDataByIdService, userInterviewDataExistsService } from '../services/interviewData.service.js';
...
import { AWSPollySynthesizeSpeechmarks, AWSPollyTextToSpeechBuffer } from '../AWS/Polly.AWS.js';
...
const startInterview = async (req, res) => {
  const { userId, level, category, subCategory, numberOfQuestions, intervieweeName, techImage } = req.body;
  if (!userId || !level || !category || !subCategory || !numberOfQuestions || !intervieweeName || !techImage) {
  ...
  const { _id: interviewDataId, interviewCredits, answeredQuestionIds, onGoingInterview, onGoingInterviewDetails } = interviewData
  // Interview is ongoing

  if (onGoingInterview) {
    const isSameCategory = onGoingInterviewDetails.filter(item => item.category === category && item.subCategory[0] === subCategory[0]).length > 0
    if (isSameCategory)
      throw new BadRequestError(`Another interview for ${subCategory[0]} is in progress, please complete it first.`);
  }
  ...
  const interviewQuestions = await fetchInterviewQuestionsFromDbService({ level, category, subCategory, numberOfQuestions, excludedIds: answeredQuestionIds })
  ...
  const promises = interviewQuestions.map(async (item) => {
    try {
      ...
      if (!lipsyncFileKey || !audioFileKey) {
        ...
        // Create File names
        const audioFileKey = `questions/v1_${level}_${category}_${subCategory}_${uid}`.split(' ').join('_') + '.mp3';
        const lipsyncFileKey = `speechMarks/v1_${level}_${category}_${subCategory}_${uid}`.split(' ').join('_') + '.json';
        // Upload to S3
        await uploadAudioBufferToS3({ name: audioFileKey, audioBuffer });
        await uploadSpeechMarksToS3({ name: lipsyncFileKey, speechMarks: JSON.stringify(speechMarks) });
        // Update question in db
        const data = { audioFileKey, lipsyncFileKey }
        const updateQuestion = await updateQuestionByIDService({ questionId: _id, data })
  ...
  const updatedInterviewData = await updateInterviewDataByIdService({ interviewDataId, data: interviewDataUpdate })
  return res.status(200).send({ interview, interviewData: updatedInterviewData });
};
...
const postAnswerFile = async (req, res) => {
  ...
  const { interviewDetails, health, numberOfQuestions, status, profanityCount, category, subCategory } = interview;
  ...
  if (profane) {
    ...
    const data = {
      ...
      $set: {
        ...
        cancelReason: profanityCount === 3 ? 'Use of profane language.' : undefined,
    ...
    if (profanityCount === 3) {
      ...
      updatedInterviewData = await updateInterviewDataByIdService({ interviewDataId: interviewData._id, data: interviewsData })
    }

    const updatedInterview = await updateInterviewByIdService({ interviewId, data });
    // Return with updated health
    return res.status(405).send({ msg: profaneText, updatedInterview, interviewData: updatedInterviewData });
  }
  ...
  const data = {
    $set: {
      currentQuestion:
        Number(answerNumber) + 1 < numberOfQuestions ? Number(answerNumber) + 1 : answerNumber,
      interviewDetails,
    },
  };
...
const endInterview = async (req, res) => {
  ...
  const { interviewDetails, level, category, subCategory, numberOfQuestions } = interview;

  // Format answers
  const formattedAnswers = formatInterviewDetailsPreEvaluationService(interviewDetails);

  // Evaluate results
  const { results, advice } = await getInterviewEvalutaionNew({ level, category, subCategory, interviewDetails: formattedAnswers, numberOfQuestions });

  // Update data
  let evaluationPercentage = 0;
  const interviewDetailsResults = await Promise.all(interviewDetails.map(async (item, i) => {
    const { asked, averageScore, questionId } = item;
    const score = getPercentage({ level, result: results[i] });
    const { feedback, clarity, accuracy, completeness, relevance, communication } = results[i]

    evaluationPercentage += score
    const oldAverageScore = Number(averageScore || 0) * (asked || 0);
    const newAverageScore = Math.floor((oldAverageScore + score) / (asked ? asked + 1 : 1));

    try {
      await updateQuestionByIDService({ questionId, data: { averageScore: newAverageScore, $inc: { asked: 1 } } });
    } catch (error) {
      console.log(error)
    }
  ...
  interview.evaluationPercentage = Math.floor(evaluationPercentage / numberOfQuestions);
  ...
  const updatedInterviewData = await updateInterviewDataByIdService({ interviewDataId, data: interviewsData })
  return res.status(200).send({ interview, interviewData: updatedInterviewData });
};



export { getAllInterviews, checkLatestInterviewData, startInterview, endInterview, postAnswerFile };
